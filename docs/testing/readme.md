## Useful Information

## Useful Information

While testing this service, we need to ensure that database logs are verified to check whether the
payment is successful
or not, as well as verify Chips.

The types of URLs in this service are extensive and can be found in
the [ecs-service-config-dev](https://github.com/companieshouse/ecs-service-configs-dev)
and [ecs-service-config-production](https://github.com/companieshouse/ecs-service-configs-production)
repository under
the `payments.api.ch.gov.uk.env` file for the `DOMAIN_ALLOW_LIST` variable for various environments.

## Manual Tests

## Automated Tests

## Performance Tests
